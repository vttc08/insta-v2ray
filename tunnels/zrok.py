...
import threading
import re
from helper.check import check_all_binaries, is_docker, check_binary_status
...
class Zrok:
    ...
    def __init__(self, host: str, port: int, **provider_kwargs):
        ...
        self.limit = 10
        self.timeout = 20 # custom timeout for zrok
        __class__.tunnels += 1
    ...
    def prerequisites(self):
        self.bin = check_all_binaries(bin_path, zrok_binary)
        # check binary status to be implmemented later
        if not self.bin:
            error = binary_not_found(zrok_binary, self.__class__.__name__)
            logger.error(error)
            raise RuntimeError(error)
    ...
    def read_stdout(self):
        ...
        for line in self.process.stdout:
            ...
            match = re.search(r"https://[^\s]+\.zrok.io", decoded_line)
            if match:
    ...
    def start_tunnel(self):
        command = f"{self.bin} share public {self.host}:{self.port}"
        ...
        if not self.tunnel_url:
            self.process.kill()
            error = tunnel_url_not_found(url="zrok.io")
            logger.error(error)
            raise RuntimeError(error)
